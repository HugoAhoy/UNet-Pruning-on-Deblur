...
def unet_origin_deps_flattened(net):
    result = []
    allpara = net.state_dict()
    for param in allpara:
        if 'conv.bias' in param:
            result.append(allpara[param].shape[0])

    return np.array(result)

def unet_pacesetter_idxes(net):
    return None

def unet_pacesetter_dict():
    return None
