...
class Engine(object):
    ...
    def set_pthVal(self, param, value):
        param.data = value.cuda()
        # set grad to None, otherwise in backward() will cause dimension mismatch
        param.grad = None
    ...
    def get_all_conv_kernel_namedvalue_as_list(self, remove = None):
        result = []
        for k, v in self.state.model.state_dict().items():
            if remove in k:
                continue
    ...
    def save_pth(self, path):
        ...
        print('---------------saved {} numpy arrays to {}---------------'.format(len(save_dict['model']), path))
    ...
    def load_pth(self, path):
        ...
        for k, v in self.state.model.named_parameters():
            ...
            if key in state_dict:
                print('assign {} from pth'.format(k))
                self.set_pthVal(v, state_dict[key])
                assigned_params += state_dict[key].numel()
            else:
                print('param {} not found in pth'.format(k))
        ...
        for k, v in self.state.model.named_buffers():
            ...
            if key in state_dict:
                ...
                print('buffer {} not found in pth'.format(k))
            assigned_params += 1
        print('Assigned {} params from pth: {}'.format(assigned_params, path))
